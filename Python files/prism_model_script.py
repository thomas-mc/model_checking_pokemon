                    ...
                    for pokemon_counter in range (1,4):
                        for move_counter in range(1,5):
                            for adv_pokemon_counter in range(1,4):
                                ...
                                for poke_type in live_stats['p1p{0}_type'.format(pokemon_counter)]:
                                    for adv_poke_type in live_stats['p2p{0}_type'.format(adv_pokemon_counter)]:
                                        type_effectiveness = type_effectiveness * type_effectiveness_list[poke_type][adv_poke_type]

                                live_damage_stats['p{0}p{1}m{2}p{3}p{4}'.format(player_counter, pokemon_counter, move_counter, 3-player_counter, adv_pokemon_counter)] = math.floor((((((2*level)/5)+2)*power*(attack/defense))/50) * random_var * stab * type_effectiveness)


                # ---------------------UNCOMMENTING THIS CODE SPAMS FILES----------------------
                f=open("University Work\\model_checking_pokemon\\Prism_Models\\{0}_VERSUS_{1}.prism".format(team,adversary_team),'w')
                sys.stdout = f
                # -----------------------------------------------------------------------------
